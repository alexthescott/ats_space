...
function shuffle(the_array){
  ...
  var new_list = the_array.slice()
  for(var i = 0; i < end; i++){
    var swap_i = randint(i, end-1) 
    var temp_var = new_list[i]
...
export default function sketch(p){
  ...
  p.setup = () => {
    sticks_count = (p.random() <= 0.5) ? 108 : 72 
    ...
    for(var c = 1; c < circle_count + 1; c++){
      var stick_angle = 0.0
      var angle_step = p.TWO_PI / (sticks_count / circle_count)
      var draw_color = palette[c]
      for(var i = 0; i < sticks_count / circle_count; i++){
        var x_pos = p.map(p.cos(stick_angle), -1, 1, p.width/10 * c, p.width-p.width/10 * c)
        var y_pos = p.map(p.sin(stick_angle), -1, 1, p.height/10 * c, p.width-p.height/10 * c)
        var pos = p.createVector(x_pos, y_pos)
  ...
  p.draw = () => {
    ...
    for (var s = 0; s < sticks_count; s++){
      sticks[s].draw()
    }
    if (p.frameCount === 1 || p.frameCount % 180 === 0){
  ...
  class stick {
    ...
    draw(){
      var x_len = (p.cos(this.angle) * this.len) / 2
      var y_len = (p.sin(this.angle) * this.len) / 2
      var s_x = this.pos.x - x_len
      var s_y = this.pos.y - y_len
      var e_x = this.pos.x + x_len
      var e_y = this.pos.y + y_len
      p.stroke(this.draw_color)
      p.line(s_x, s_y, e_x, e_y)
      this.update()
    }
    ...
    update(){
      ...
      if (this.start_t < this.end_t){
        ...
        var x_dist = p.map(percent, 0, 1, this.pos_start.x, this.goal.x)
        var y_dist = p.map(percent, 0, 1, this.pos_start.y, this.goal.y)
        var angle_dist = p.map(percent, 0, 1, this.angle, this.goal_angle)
        this.pos.x = x_dist
        this.pos.y = y_dist
        ...
        var new_r = p.map(percent, 0, 1, cur_r, goal_r)
        var new_g = p.map(percent, 0, 1, cur_g, goal_g)
        var new_b = p.map(percent, 0, 1, cur_b, goal_b)
